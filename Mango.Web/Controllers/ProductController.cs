...
namespace Mango.Web.Controllers
{
    public class ProductController : Controller
    {
        ...
        public async Task<IActionResult> ProductIndex()
        {
            ...
            var response = await _productService.GetAllProductsAsync<ProductResponseDto>(accessToken);
        ...
        public async Task<IActionResult> ProductCreate(ProductRequestDto model)
        {
            ...
            {
                var accessToken = await HttpContext.GetTokenAsync("access_token");
                var response = await _productService.CreateProductAsync<ProductResponseDto>(model, accessToken);
                if (response != null && response.IsSuccess)
                {
        ...
        {
            var accessToken = await HttpContext.GetTokenAsync("access_token");
            var response = await _productService.GetProductByIdAsync<ProductResponseDto>(productId, accessToken);
            if (response != null && response.IsSuccess)
            {
        ...
        public async Task<IActionResult> ProductEdit(ProductRequestDto model)
        {
            ...
            {
                var accessToken = await HttpContext.GetTokenAsync("access_token");
                var response = await _productService.UpdateProductAsync<ProductResponseDto>(model, accessToken);
                if (response != null && response.IsSuccess)
                {
        ...
        public async Task<IActionResult> ProductDelete(ProductRequestDto model)
        {
            ...
            {
                var accessToken = await HttpContext.GetTokenAsync("access_token");
                var response = await _productService.DeleteProductAsync<ProductResponseDto>(model.ProductId, accessToken);
